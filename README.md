...
- Subtraction of whole numbers
  - subtraction as an adaptation of addition algorithm, by performing the following steps:
    - In the general case, cancel the leading digit of the first operand, and create a third row of numbers on top of the first operand, in which the first number of the row is 1 less than the leading digit of the first operand, the last number of the row is 10, and the intermediate numbers are each 9.
    - Then finish of the subtraction, by performing addition with the standard addition algorithm involving 3 rows of numbers and treating each digit of the second operand as negative (although student would not think in terms of addition with negatives, but rather subtraction of second operand digit).
    - Optimizations:
      - if operands have the same number of digits, then ignore or cancel out common leading digits of operands.
      - if each digit of first operand is not less than corresponding digit of second operand, then skip the creation of a third row above the first operand.
    - NB: this method is inspired by the shopkeeper's method (aka count up method, count forward method), in that it avoids the need for the student to know or remember that subtraction is not commutative, and it completely avoids the step of borrowing present in the standard subtraction algorithm. Instead, it only requires the student to know how to compute absolute differences of small operands (ie number whose magnitude is less than 20), and how to compute additions and comparisons for large numbers.
