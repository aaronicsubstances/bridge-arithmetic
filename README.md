...
- Alternative/Precursor to standard subtraction procedure of whole numbers based on borrowing
  ...
  - Variation of Ten's complement method - Like shopkeeper's method, ten's complement method omits borrowing and avoids the need for the student to know or remember that subtraction is not commutative. This variation introduces borrowing to make it resemble standard procedure based on borrowing. It also utilises both the space above the first operand used by the standard procedure based on borrowing, and the space below the second operand used by the Austrian method. It works as follows: begin by cancelling out the common prefix of both operands. If no digits remain after this step, then it means the operands were equal and hence the final answer is zero. else if there is a single digit left in first operand, perform subtraction directly and the result is the final answer. else first operand will be left with at least two digits. create a row above first operand, consisting of 10 written out as 9+1 in the last position, 9 in all positions but the first and last positions, and in the first position one less than the first digit of the first operand. cancel the first digit of the first operand. then subtract the second operand from the newly created row digit by digit, skipping the +1, and fill the space below the second operand with the result. to make transitioning to optimized procedures easier, start subtraction from last digit of second operand. then cancel all the digits of the second operand. the final answer is the sum of the first operand, the +1 in the row above the first operand, and the result below the second operand.
